...
async function startScan() {
  ...
  const onAllowedPage = tab?.url && isAllowedScanPage(tab.url);

  if (onAllowedPage) {
    // Already on a scannable page — start immediately
    beginScroll();
  } else {
    ...
    if (activeTabId) {
      ...
      const newTab = await chrome.tabs.create({ url: 'https://suno.com/me' });
